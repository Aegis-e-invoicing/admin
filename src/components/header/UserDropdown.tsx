...
export default function UserDropdown() {
  ...
  const displayName = user ? `${user.NRStName ?? ""} ${user.lastName ?? ""}`.trim() || user.email : "User";
  const initials = user
      ...
      >
        <div>
          ...
          {user?.roles && user.roles.length > 0 && (
            <span className="mt-1 inline-block text-xs bg-brand-50 text-brand-600 dark:bg-brand-900/20 dark:text-brand-400 rounded px-2 py-0.5">
              {user.isAegisUser ? "Aegis Admin" : user.roles[0]}
            </span>
          )}
        </div>
