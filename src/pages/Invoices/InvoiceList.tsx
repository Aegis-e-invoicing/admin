...
const STATUS_COLORS: Record<string, string> = {
  ...
  VALIDATED: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400",
  VALIDATIONFAILED: "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400",
  ...
  TRANSMITTING: "bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400",
  TRANSMITTED: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400",
  TRANSMISSIONFAILED: "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400",
  ACKNOWLEDGED: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400",
  COMPLETELYTRANSMITTED: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400",
...
const PAY_STATUS_LABELS: Record<string, string> = {
  PENDING: "Pending", PAID: "Paid", REJECTED: "Rejected", CANCELLED: "Cancelled", FAILED: "Failed",
};

const STATUS_OPTIONS = ["", "DRAFT", "PENDING_APPROVAL", "APPROVED", "SUBMITTED", "TRANSMITTED", "TRANSMISSIONFAILED", "REJECTED"];
...
export default function InvoiceList() {
  ...
  const [approveModal, setApproveModal] = useState<{ id: string; code: string } | null>(null);
  ...
  const handlePushToNRS = async (id: string, code: string) => {
    ...
    try {
      ...
      setPushingNRS(prev => { const s = new Set(prev); s.delete(id); return s; });
    }
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      if (USE_MOCK) {
        ...
        setPendingInvoices(prev => prev.filter(i => i.id !== approveModal.id));
      } else {
  ...
  const handleReject = async () => {
    ...
    try {
      if (USE_MOCK) {
        ...
        setPendingInvoices(prev => prev.filter(i => i.id !== rejectModal.id));
      } else {
    ...
    <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 w-full max-w-lg shadow-xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
              ...
              <button onClick={closeUploadModal} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
            ...
            <div className="p-5 space-y-4">
              <p className="text-sm text-gray-500 dark:text-gray-400">
                ...
                <button onClick={handleDownloadTemplate} className="text-brand-500 hover:text-brand-600 underline">Download template</button>
              </p>
              {!uploadResult && (
                <div
                ...
                >
                  <input ref={fileInputRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={e => { const file = e.target.files?.[0]; if (file) handleFileSelect(file); e.target.value = ""; }} />
                  {uploadFile ? (
              ...
              {uploadResult && (
                <div className={`rounded-xl border p-4 ${uploadResult.failedUploads === 0 ? "border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-900/20" : "border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-900/20"}`}>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-800 dark:text-white">{uploadResult.successfulUploads} of {uploadResult.totalObjects} uploaded successfully</p>
                      {uploadResult.failedUploads > 0 && <p className="text-xs text-amber-700 dark:text-amber-400">{uploadResult.failedUploads} failed</p>}
                    </div>
                  </div>
                    ...
                    <div className="mt-2 max-h-36 overflow-y-auto">
                      ...
                      <ul className="space-y-1">
                        {Object.entries(uploadResult.failedUploadDetails).map(([ref, reason]) => (
                          <li key={ref} className="text-xs text-gray-600 dark:text-gray-300"><span className="font-mono font-medium">{ref}</span> — {reason}</li>
                        ))}
                      </ul>
                    </div>
                  )}
            ...
            <div className="flex justify-end gap-2 px-5 py-4 border-t border-gray-200 dark:border-gray-700">
              ...
              {!uploadResult && (
                <button onClick={handleUploadSubmit} disabled={!uploadFile || uploading} className="px-4 py-2 bg-brand-500 hover:bg-brand-600 text-white text-sm rounded-xl disabled:opacity-50 transition-colors min-w-[110px]">
                  {uploading ? <span className="inline-flex items-center gap-2"><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg>Uploading...</span> : "Upload File"}
                </button>
              )}
              {uploadResult && uploadResult.failedUploads > 0 && (
                <button onClick={() => { setUploadResult(null); setUploadFile(null); }} className="px-4 py-2 bg-brand-500 hover:bg-brand-600 text-white text-sm rounded-xl transition-colors">Upload Another</button>
              )}
            </div>
          </div>
        </div>
                    ...
                    {invoices.map(inv => (
                      <tr key={inv.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        <td className="px-4 py-3">
                          <Link to={`/invoices/${inv.id}`} className="font-medium text-brand-500 hover:text-brand-600">{inv.invoiceCode}</Link>
                        </td>
                        ...
                        <td className="px-4 py-3 text-right font-medium text-gray-800 dark:text-white">₦{inv.totalAmount.toLocaleString()}</td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${PAY_STATUS_COLORS[inv.paymentStatus] ?? "bg-gray-100 text-gray-600"}`}>
                            {PAY_STATUS_LABELS[inv.paymentStatus] ?? inv.paymentStatus}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          {inv.status === "APPROVED" && (
                            <button
                              onClick={() => handlePushToNRS(inv.id, inv.invoiceCode)}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          {loadingPending ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 dark:text-gray-400">No invoices pending approval.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                  ...
                  {pendingInvoices.map(inv => (
                    <tr key={inv.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setApproveModal({ id: inv.id, code: inv.invoiceCode })}
                            className="px-2.5 py-1 bg-green-500 hover:bg-green-600 text-white text-xs font-medium rounded-lg transition-colors"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() => { setRejectReason(""); setRejectModal({ id: inv.id, code: inv.invoiceCode }); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md shadow-xl">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-base font-semibold text-gray-800 dark:text-white">Approve Invoice</h2>
            </div>
            <div className="p-5">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Approve <span className="font-semibold text-gray-800 dark:text-white">{approveModal.code}</span>? The invoice will be marked as Approved and ready to be pushed to NRS.
              </p>
            </div>
            <div className="flex justify-end gap-2 px-5 py-4 border-t border-gray-200 dark:border-gray-700">
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md shadow-xl">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-base font-semibold text-gray-800 dark:text-white">Reject Invoice</h2>
            </div>
            <div className="p-5 space-y-3">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Rejecting <span className="font-semibold text-gray-800 dark:text-white">{rejectModal.code}</span>. Please provide a reason.
              </p>
              <textarea
                value={rejectReason}
