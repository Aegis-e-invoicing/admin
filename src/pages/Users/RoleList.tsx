    ...
    <>
      ...
      {showForm && (
        <div className="fixed inset-0 z-9999999 flex" aria-modal="true" role="dialog">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => { setShowForm(false); setForm(emptyForm); }} />
          <div className="relative ml-auto w-full max-w-xl h-full bg-white dark:bg-gray-900 shadow-2xl flex flex-col">
            <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
              <div>
                <h2 className="text-base font-semibold text-gray-800 dark:text-white">New Custom Role</h2>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Assign a name and permissions for this role</p>
              </div>
              <button type="button" onClick={() => { setShowForm(false); setForm(emptyForm); }} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500">✕</button>
            </div>
                      ...
                      return (
                        <div key={group.group}>
                          <label className="flex items-center gap-2 mb-2 cursor-pointer">
                            <GroupCheckbox checked={allSelected} indeterminate={someSelected}
                            ...
                            />
                            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{group.group}</span>
                          </label>
                          <div className="space-y-1.5 pl-6">
                            {group.items.map((item) => (
                              <label key={item.value} className="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" checked={form.permissions.includes(item.value)} onChange={() => toggleFormPerm(item.value)}
      ...
      {editRole && (
        <div className="fixed inset-0 z-9999999 flex" aria-modal="true" role="dialog">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setEditRole(null)} />
          <div className="relative ml-auto w-full max-w-xl h-full bg-white dark:bg-gray-900 shadow-2xl flex flex-col">
            <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
              <div>
                <h2 className="text-base font-semibold text-gray-800 dark:text-white">Edit Permissions — {editRole.name}</h2>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{editRole.description}</p>
              </div>
              <button type="button" onClick={() => setEditRole(null)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500">✕</button>
            </div>
                ...
                {ASSIGNABLE_PERMISSIONS.map((group) => {
                  ...
                  return (
                    <div key={group.group}>
                      <label className="flex items-center gap-2 mb-2 cursor-pointer">
                        <GroupCheckbox checked={allSelected} indeterminate={someSelected}
                          onChange={(on) => setEditPerms((prev) =>
                            on ? [...new Set([...prev, ...groupValues])] : prev.filter((p) => !groupValues.includes(p))
                          )}
                        />
                        <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{group.group}</span>
                      </label>
                      <div className="space-y-1.5 pl-6">
                        {group.items.map((item) => (
                          <label key={item.value} className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={editPerms.includes(item.value)} onChange={() => toggleEditPerm(item.value)}
                              className="w-4 h-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500" />
                            <span className="text-xs text-gray-700 dark:text-gray-300">{item.label}</span>
                          </label>
                        ))}
